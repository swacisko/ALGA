    ...
    static void writeProgress( int processed, int toProcess, int & progressCounter, string text, int accuracy = 1 ){
        if( toProcess < 0 ){
           if( processed % accuracy == 0 ) {
            ...
            while( accuracy*100*(double)processed/*/(double)( toProcess )*/ >= progressCounter * (double)( toProcess ) ){
                if( accuracy == 1 ) cerr << "\r" << text << ": " << progressCounter << " %" << flush;
                else cerr << "\r" << text << ": " << ((double)progressCounter / max( (double)accuracy, 0.000001) ) << " %" << flush;
                progressCounter++;
            }
        }
    }

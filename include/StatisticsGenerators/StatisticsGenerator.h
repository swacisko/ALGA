...
//        if( Params::WRITE_STATISTICS == false ) return;
        ...
        map<string, double> stats;
        stats["number of elements"] = accumulate(data.begin(), data.end(), 0ll,
                                                 [](auto a, auto b) { return a + b.second; });
        stats["sum of elements"] = accumulate(data.begin(), data.end(), 0ll,
                                              [](auto a, auto b) { return a + b.first * b.second; });
        stats["average"] = stats["sum of elements"] / stats["number of elements"];
        stats["minVal element"] = accumulate(data.begin(), data.end(), 1000000000ll,
                                             [](auto a, auto b) { return min(a, b.first); });
        stats["maxVal element"] = accumulate(data.begin(), data.end(), 0ll,
                                             [](auto a, auto b) { return max(a, b.first); });

        double avg = stats["average"];
        stats["standard deviation"] = sqrt((double) accumulate(data.begin(), data.end(), 0ll, [&avg](auto a, auto b) {
            return a + b.second * (b.first - avg) * (b.first - avg);
        }) / stats["number of elements"]);
        stats["sum of squares of elements"] = accumulate(data.begin(), data.end(), 0ll, [](auto a, auto b) {
            return a + b.second * b.first * b.first;
        });

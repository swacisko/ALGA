...
public:
    ...
    void changeId( int newId ){ id = newId; } // changes id to newID. This is used to remove duplicate reads from all READS and assign new id's to those reads

    bool operator<( Read & oth ){ /*if( *sequence != *oth.sequence ) return (*sequence) < (*oth.sequence); else */return id < oth.id; }
...
//    bool operator==( const Read & oth );
    ...
    int getIdOfCompRevRead(); // returns id of a read that is reverse complementary of this one or -1 if we did not add reverse complemetary.
    int getIdOfPairedRead(); // return id of a read that is paired with this one or -1 if we do not have such information.

    static int getIdOfPairedRead(int id);
    static int getIdOfCompRevRead(int id); // returns id of a read that is reverse complementary of this one or -1 if we did not add reverse complemetary.

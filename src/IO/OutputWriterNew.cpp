...
void OutputWriterNew::writeContigs() {
    ...
    if (Params::TRAVERSE_TYPE == Params::TRAVERSE_GREEDY)
        traverse_type += "_ccscl" + to_string(Params::CONTIG_CREATOR_SHORT_CYCLE_LENGTH);

    string rsoe = (to_string(Params::REMOVE_SMALL_OVERLAP_EDGES_MIN_OVERLAP) + "-" +
    ...
    sort(contigs.begin(), contigs.end(), [](auto &c1, auto &c2) {
        return c1->size() > c2->size();
    }); // after this contigs should be ordered from longest to shortest.
...
void OutputWriterNew::writeContigsNoFilter(vector<Contig *> contigs) {
    Params::outStream.open(Params::outStreamFileName
            /*+ "_nrpcp" + to_string(Params::NEW_READS_PER_CONTIG_PERCENTAGE) + "_mopp" + to_string(Params::MAX_OFFSET_PARALLEL_PATHS)
            + "_modb" + to_string(Params::MAX_OFFSET_DANGLING_BRANCHES) + rsoe + traverse_type + "_contigs.fasta"*/
    );

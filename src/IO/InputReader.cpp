...
void InputReader::readInput() {
    ...
    cerr << "There are " << 2*cnt << " read with " << 1 << " N's in the middle (at least 10 from end) of the read" << endl;
        ...
        while(true){
            ...
            for( int i=0; i<Params::THREADS; i++ ){
                if( p < parallelReads[i].size() ){
                    if( p < 0 || p >= parallelReads[i].size() ){ cerr << "invalid p in merge reads to Global::READS" << endl; exit(1); }
...
//                readsWithNSizes.push_back( countN );
            ...
            if( p < s.size() ) s.erase( p, s.size() - p+1 );
             ...
             if( Params::ADD_COMP_REV_READS ) Global::addRead(MyUtils::getComplimentaryString(MyUtils::getReverse(s)), Global::READS);
...
void InputReader::readParallelJob( vector< vector<Read *> > &reads, int thread_id) {
    ...
    while( ( s = readOneRead1(str) ) != "" ){
        ...
        if( thread_id < 0 || thread_id >= reads.size() ){ cerr << "invalid thread_id in readParallelJobs" << endl; exit(1);  }

        if( s.size() < Params::READ_END_TRIM_LEFT + Params::READ_END_TRIM_RIGHT + 10 ){
//            cerr << "s = " << s << endl;
        }else{
            s.erase( s.begin(), s.begin() + min( Params::READ_END_TRIM_LEFT, (int) s.size()  ) ); // these two lines here were used, to produce best results!!! (porbably ends are not as good as they ought to be, even after trimmomatic).

...
GraphCreatorPairwiseKmerBranch::GraphCreatorPairwiseKmerBranch( vector<Read*> *reads, Graph * G  ) : GraphCreatorKmerBased(reads,G) {
...
void GraphCreatorPairwiseKmerBranch::createAlignmentsForKmers(vector<Kmer> &kmers, int p, int q, int thread_id) {
    ...
    if( branchMarkers.size() < D || ( branchMarkers.size() > LARGE_BUCKET_SIZE && D < LARGE_BUCKET_SIZE ) ) branchMarkers = vector<Bitset>( D, Bitset(D) );
    else{
    ...
    for( int i=q-1; i>=p; i-- ){
        ...
        for( int j=i+1; j<= min( q, i + WINDOW_SEARCH_SIZE ) ; j++ ){
            ...
            if( branchMarkers[i-p][j-p] == 0 ){ // if i cannot yet get from id1 to id2
                ...
                if( neighbors[id2] != Params::INF ){ // if there was and edge, but longer than current offset, or i just added it, i mark branchMarkers

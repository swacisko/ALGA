 ...
 */
Kmer::Kmer(Read *r, int ind, Params::KMER_HASH_TYPE hash, short length) : read(r), indInRead(ind) /*hash(hash + Params::KMER_HASH_ADDITION) */{
    this->hash = hash;
    if( hash < 0 ){
...
ostream& operator<<( ostream& str, Kmer & k ){
    str << "Kmer: " << k.getKmerAsString() << "  with (LL)hash  " << (LL)k.hash << "  at index " << k.indInRead << "   with length " << k.length << "  in read " << (*k.read);
    return str;
}

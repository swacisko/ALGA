...
void Graph::mergeVertices(int a, int b, int offset) {
        ...
        for( int i=0; i<V[a].size(); i++ ){
            if( V[a][i].first != b && V[a][i].first != a ) addDirectedEdge( b, V[a][i].first, V[a][i].second ); // i do not want to add edge (b,b)
        }
    ...
    for( int i=0; i<size(); i++ ){
        ...
        if( neigh.size() != othNeigh.size() ){
            ...
            for( int k=0; k<neigh.size(); k++ ){
                cerr << "neigh[k] = (" << neigh[k].first << "," << neigh[k].second << ")   othNeigh[k] = (" << othNeigh[k].first << "," << othNeigh[k].second << ")" << endl;
            }
            return false;
        }
        for( int k=0; k<neigh.size(); k++ ){
...
void Graph::retainOnlySmallestOffsetJob(int a, int b, int thread_id) {
    ...
    for( int i=a; i <= b; i++ ){
        ...
        swap( V[i], newNeigh );
        newNeigh.clear();
        if(thread_id == 0) MyUtils::writeProgress( i-a+1 , b-a+1 , progressCounter, "retaining smallest offset for same edge in thread 0",1);
    }
}
...
void Graph::getReverseGraphJob(int a, int b, int thread_id, Graph &GRev) {
    ...
    for( int i=a; i<=b; i++ ){
        ...
        if( thread_id == 0 ) MyUtils::writeProgress(i+1,size() / Params::THREADS, progressCounter, "creating reverse graph",1);
    }
}
...
void Graph::sortEdgesByIncreasingOffsetJob(int a, int b, int thread_id) {
    int progressCounter = 0;
    for(int i=a; i<=b; i++){
        sort( V[i].begin(), V[i].end(), [](auto &p, auto &q){ if( p.second != q.second ) return p.second < q.second; else return p.first < q.first;      } );
        if(thread_id == 0) MyUtils::writeProgress(i+1,size() / Params::THREADS, progressCounter, "sorting edges by minimal offset",1);
    }
}
...
LL Graph::countEdges() {
    ...
    for( int i=1; i<Params::THREADS; i++ ){
        ...
        int b = min( (i+1)*W-1, (int)size()-1 );
          ...
          futures[i-1] = std::async(std::launch::async, [=](int a, int b, int i){ return countEdgesJob(a,b,i); }, a,b,i );
    }
